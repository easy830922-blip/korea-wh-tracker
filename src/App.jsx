...
const TASKS_INIT = [
  ...
  { id: "t04", title: "拍攝白底彩色證件照", desc: "3.5 × 4.5cm，最近 6 個月內拍攝，用於貼在簽證申請書上", deadline: "2026-05-09", assignee: "both", category: "證件照", phase: 2 },
  ...
  { id: "t09", title: "投保海外旅平險", desc: "保期 ≥ 1 年（365 天），需含海外事故意外理賠，保單須明載「海外 / overseas / global」字樣", deadline: "2026-05-14", assignee: "both", category: "保險", phase: 3 },
  { id: "t10", title: "撰寫打工渡假行程及計畫書", desc: "下載代表部附件格式，中英韓文均可，務必親筆簽名", deadline: "2026-05-16", assignee: "both", category: "計畫書", phase: 4 },
  { id: "t11", title: "填寫簽證申請書", desc: "建議英文或韓文填寫，貼上證件照，個人資料 / 護照 / 聯絡 / 訪問資訊 / 聲明", deadline: "2026-05-16", assignee: "both", category: "申請書", phase: 4 },
  ...
  { id: "t14", title: "按代表部順序排列全部 10 項文件", desc: "順序：護照→申請書→影本→戶籍→存款→計畫書→學歷→良民證→健檢→保險", deadline: "2026-05-29", assignee: "both", category: "最終確認", phase: 6 },
...
const INFO_SECTIONS = [
  { title: "申請資格", items: [
    ["年齡限制", "滿 18 歲以上 ~ 34 歲以下（含），以申請日為基準"],
    ["適用對象", "中華民國（臺灣）國民"],
    ["限制", "一生僅能申請一次，曾發給此簽證者不得再申請"],
  ]},
  { title: "申請方式", items: [
    ["收件期間", "2026.5.5（二）~ 12.4（五）平日"],
    ["收件時間", "08:30-11:20 / 14:00-15:50"],
    ["名額", "每年 800 位，按順序受理額滿為止"],
    ["申請方式", "本人親自至代表部申請，備齊文件當場受理"],
    ["審查時間", "約 10 個工作天"],
    ["領件", "憑收據至代表部領取，可由他人代領"],
  ]},
  { title: "簽證內容", items: [
    ...
    ["有效期", "核發日起 1 年內多次入境"],
    ["停留期限", "入境日起最長 1 年"],
    ["外國人登錄證", "停留 90 天以上須於入境後 90 天內申請"],
  ]},
  { title: "就業限制", items: [
    ["工時", "一年 ≤ 1,300 小時（每週 25 小時）"],
    ["雇用方式", "不限約聘 / 時薪，符合工時即可"],
    ["禁止職業", "教授、外語教學、律師、醫師、機師、研究員等專業職"],
    ["禁止場所", "聲色場所"],
  ]},
  { title: "代表部資訊", items: [
    ["地址", "台北市基隆路一段 333 號 1506 室（國貿大樓 15F）"],
    ["電話", "(02) 2758-8320~5"],
    ["休館日", "6/19、7/17、9/25、9/28、10/9、10/26、12/25"],
  ]},
  { title: "繳件文件順序", items: [
    ["①", "護照正本（效期 ≥ 12 個月）+ 身分證正本"],
    ["②", "簽證申請書（貼 3.5×4.5cm 彩色照片）"],
    ["③", "護照 + 身分證影本各 1 份"],
    ["④", "全戶戶籍謄本"],
    ["⑤", "存款餘額證明（USD $3,000+，本人名義）"],
    ["⑥", "打工渡假行程及計畫書（須簽名）"],
    ["⑦", "學歷證明正本 + 影本（或在學證明正本）"],
    ["⑧", "無犯罪紀錄證明（良民證）"],
    ["⑨", "健康診斷書（衛福部指定醫院）"],
...
export default function App() {
  ...
  const [tasks, setTasks] = useState(() => {
    if (saved?.tasks) {
      return TASKS_INIT.map(t => {
        ...
        return s ? { ...t, erinDone: s.erinDone||false, henryDone: s.henryDone||false, erinFile: s.erinFile||"", henryFile: s.henryFile||"" } : { ...t, erinDone: false, henryDone: false, erinFile: "", henryFile: "" };
      });
    }
    return TASKS_INIT.map(t => ({ ...t, erinDone: false, henryDone: false, erinFile: "", henryFile: "" }));
  });
  ...
  useEffect(() => { save({ tasks: tasks.map(t => ({ id: t.id, erinDone: t.erinDone, henryDone: t.henryDone, erinFile: t.erinFile, henryFile: t.henryFile })) }); }, [tasks]);
  ...
  const onFile = (e) => {
    ...
    setTasks(prev => prev.map(t => t.id === uploadCtx.id ? { ...t, [`${uploadCtx.who}File`]: f.name } : t));
    setToast(`${P[uploadCtx.who].name}「${tasks.find(t=>t.id===uploadCtx.id)?.title}」檔案已記錄 — 請同步上傳至 Google Drive`);
  ...
  const phaseStats = useMemo(() => PHASES.map(p => {
    const pts = tasks.filter(t => t.phase === p.n);
    let tot = 0, dn = 0;
    pts.forEach(t => {
      const subs = t.assignee === "both" ? ["erin","henry"] : [t.assignee];
      subs.forEach(w => { tot++; if (t[`${w}Done`]) dn++; });
    });
  ...
  const filteredTasks = useMemo(() => {
    ...
    if (filter === "pending") r = r.filter(t => {
      const subs = t.assignee === "both" ? ["erin","henry"] : [t.assignee];
      return subs.some(w => !t[`${w}Done`]);
    });
    if (filter === "overdue") r = r.filter(t => {
      const subs = t.assignee === "both" ? ["erin","henry"] : [t.assignee];
      return subs.some(w => !t[`${w}Done`]) && daysLeft(t.deadline) < 0;
    });
    if (searchQ.trim()) {
      const q = searchQ.toLowerCase();
      r = r.filter(t => t.title.toLowerCase().includes(q) || t.category.toLowerCase().includes(q) || t.desc.toLowerCase().includes(q));
    }
    return r;
  }, [tasks, filter, searchQ]);

  const reset = () => { if (confirm("確定重置所有進度？")) setTasks(TASKS_INIT.map(t => ({ ...t, erinDone: false, henryDone: false, erinFile: "", henryFile: "" }))); };

  return (
    <div style={S.root}>
      <style dangerouslySetInnerHTML={{__html: `
        .bar{height:4px;border-radius:2px;transition:width .5s ease}
        .ring-track{fill:none;stroke:var(--color-border-tertiary);stroke-width:5}
        .ring-val{fill:none;stroke-width:5;stroke-linecap:round;transition:stroke-dashoffset .7s ease}
        .fade-in{animation:fadeIn .35s ease both}
        @keyframes fadeIn{from{opacity:0;transform:translateY(6px)}to{opacity:1;transform:none}}
        .hovcard{transition:box-shadow .2s ease, transform .15s ease}
        .hovcard:hover{box-shadow:0 2px 12px rgba(0,0,0,.06);transform:translateY(-1px)}
      `}} />
      ...
      <nav style={S.nav}>
        <div style={S.navInner}>
          ...
          <div style={S.navLinks}>
            {[["dashboard","Dashboard"],["tasks","Tasks"],["info","Info"]].map(([id,lb]) => (
      ...
      <main style={S.main}>
        ...
        {view === "tasks" && <Tasks tasks={filteredTasks} allTasks={tasks} toggle={toggle} filter={filter} setFilter={setFilter} searchQ={searchQ} setSearchQ={setSearchQ} handleUpload={handleUpload} stats={stats} />}
        {view === "info" && <Info />}
      </main>
  ...
  return (
    <div className="fade-in">
      ...
      <div style={S.dashHead}>
        <div>
          <h1 style={S.dashTitle}>Erin & Henry</h1>
          <p style={S.dashSub}>2026 Korea Working Holiday Tracker</p>
        </div>
      ...
      {stats.overdue > 0 && (
        <div style={S.alertBox}>
          <span style={{fontWeight:600}}>{stats.overdue} overdue task{stats.overdue>1?"s":""}</span> — review and complete as soon as possible
        </div>
      )}

      {/* Phase progress */}
      <div style={S.section}>
        <h2 style={S.secTitle}>Phase progress</h2>
        <div style={S.phaseGrid}>
          {phaseStats.map(p => (
            <div key={p.n} className="hovcard" style={S.phaseCard}>
              ...
              <div style={{height:4,borderRadius:2,background:"var(--color-border-tertiary)"}}>
                <div className="bar" style={{width:`${p.pct}%`,background: p.pct===100?"#4A9A6B":"#C96B5B"}} />
              </div>
      ...
      {/* Upcoming */}
      <div style={S.section}>
        <h2 style={S.secTitle}>Upcoming tasks</h2>
...
function MetricCard({ label, value, sub, pct, color }) {
  return (
    <div className="hovcard" style={S.mCard}>
      ...
      <div style={{height:3,borderRadius:2,background:"var(--color-border-tertiary)",marginTop:10}}>
...
function CompactTask({ t, toggle }) {
  ...
  const subs = t.assignee==="both"?["erin","henry"]:[t.assignee];
  const allDone = subs.every(w=>t[`${w}Done`]);
  return (
    <div style={{...S.compactRow, opacity: allDone?.45:1}}>
      ...
      <div style={{display:"flex",gap:4}}>
        {subs.map(w => (
          <button key={w} onClick={()=>toggle(t.id,w)} style={{...S.checkBtn, background: t[`${w}Done`] ? P[w].accent : "transparent", borderColor: t[`${w}Done`] ? P[w].accent : "var(--color-border-secondary)", color: t[`${w}Done`] ? "#fff" : "var(--color-text-tertiary)"}}>
            {t[`${w}Done`] ? "✓" : P[w].name[0]}
          </button>
        ))}
      </div>
...
/* ===== TASKS ===== */
function Tasks({ tasks, allTasks, toggle, filter, setFilter, searchQ, setSearchQ, handleUpload, stats }) {
  ...
  return (
    <div className="fade-in">
      ...
      {grouped.map(g => (
        <div key={g.n} style={{marginBottom:28}}>
          ...
          {g.items.map(t => <TaskCard key={t.id} t={t} toggle={toggle} handleUpload={handleUpload} />)}
        </div>
      ))}
...
function TaskCard({ t, toggle, handleUpload }) {
  const [open, setOpen] = useState(false);
  const dl = daysLeft(t.deadline);
  const subs = t.assignee==="both"?["erin","henry"]:[t.assignee];
  const allDone = subs.every(w=>t[`${w}Done`]);
  return (
    <div className="hovcard" style={{...S.taskCard, borderLeft: `3px solid ${allDone?"#4A9A6B":dl<0?"#C0453A":"var(--color-border-tertiary)"}` }}>
      <div onClick={()=>setOpen(!open)} style={{cursor:"pointer",display:"flex",gap:12,alignItems:"flex-start"}}>
        ...
        <div style={{display:"flex",gap:4,flexShrink:0}}>
          {subs.map(w => (
            <button key={w} onClick={e=>{e.stopPropagation();toggle(t.id,w)}} style={{...S.checkBtn, width:28,height:28, background: t[`${w}Done`]?P[w].accent:"transparent", borderColor: t[`${w}Done`]?P[w].accent:"var(--color-border-secondary)", color: t[`${w}Done`]?"#fff":"var(--color-text-tertiary)", fontSize:11}}>
              {t[`${w}Done`]?"✓":P[w].name[0]}
            </button>
          ))}
        </div>
      ...
      {open && (
        <div style={{marginTop:12,paddingTop:12,borderTop:"1px solid var(--color-border-tertiary)"}}>
          ...
          {subs.map(w => (
            <div key={w} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderTop:"1px solid var(--color-border-tertiary)"}}>
              <span style={{fontSize:12,fontWeight:600,color:P[w].accent,width:48}}>{P[w].name}</span>
              <span style={{fontSize:11,flex:1,color:"var(--color-text-tertiary)",minWidth:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                {t[`${w}File`] ? t[`${w}File`] : "No file attached"}
              </span>
              <button onClick={()=>handleUpload(t.id,w)} style={S.uploadBtn}>{t[`${w}File`]?"Replace":"Attach file"}</button>
            </div>
          ))}
        </div>
      )}
...
function Info() {
  ...
  const filtered = INFO_SECTIONS.map(sec => ({
    ...sec,
    items: sec.items.filter(([k,v]) => !q.trim() || k.toLowerCase().includes(q.toLowerCase()) || v.toLowerCase().includes(q.toLowerCase())),
    ...
    <div className="fade-in">
      ...
      {filtered.map((sec,i) => (
        <div key={i} style={{marginTop:24}}>
          <h3 style={{fontSize:13,fontWeight:600,color:"var(--color-text-primary)",marginBottom:10,letterSpacing:.3}}>{sec.title}</h3>
          <div style={S.infoTable}>
            {sec.items.map(([k,v],j) => (
              <div key={j} style={{display:"flex",padding:"10px 14px",borderBottom: j<sec.items.length-1 ? "1px solid var(--color-border-tertiary)" : "none",gap:12}}>
...
const S = {
  ...
  toast: { position:"fixed", top:12, left:"50%", transform:"translateX(-50%)", zIndex:999, padding:"10px 20px", borderRadius:8, background:"var(--color-text-primary)", color:"var(--color-background-primary)", fontSize:12, fontWeight:500, boxShadow:"0 8px 30px rgba(0,0,0,.15)" },
